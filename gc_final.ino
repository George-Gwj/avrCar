...
uint8_t deltaPWM = 160;

//初始化目标速度
uint16_t expectedPWML = SPEED - theta; // 左轮目标速度下降
uint16_t expectedPWMR = SPEED;

//采集到黑线时，传感器为低电平
uint8_t gS1 = 1;
uint8_t gS2 = 1;
uint8_t thresholdL = 140;
uint8_t thresholdR = 140;
...
const uint8_t de_speed = 90;    // 识别色卡后减速幅度
const uint8_t total_delay = 23; // 总延时时间 23s
 ...
 * 马达驱动
 * ***************************/
void motorRun(uint16_t left, uint16_t right)
{
...
//    turnLeft();
  ...
  if (gS1 == 1 && gS2 == 0)
  {
    motorRun(expectedPWML + deltaPWM, expectedPWMR - deltaPWM - theta); 
//    turnRight();
  }
  //爬坡和爬楼梯
  if (gS1 == 0 && gS2 == 0)
  {
    motorRun(expectedPWML + 20  , expectedPWMR + 10); // 左轮比右轮快10
//    down();
  }
}
